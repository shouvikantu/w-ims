...
const Login = ({ onLogin }) => {
  ...
  const signIn = () =>{
    ...
    .then((result)=>{
      onLogin(result.user); // call onLogin with user data
    })
    ...
    <div className="bg-cover bg-center bg-container">
      <div className="bg-gray-800 bg-opacity-75 min-h-screen flex items-center justify-center">
        <div className="bg-gray-300 p-8 rounded-lg shadow-lg max-w-sm w-full">
          ...
          <button onClick={signIn} className="flex items-center justify-center bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 transition-all">
