      ...
      {events.length > 0 ? (
          ...
          {events.map((event) => (
            <div className="bg-white bg-opacity-80">
              <li key={event.id} className=" rounded-lg shadow-lg p-4 w-[300px] lg:p-6 bg-gray border-2">
              <h2 className="text-xl font-bold text-black mb-2 ">{event.eventName}</h2>
              <p className="text-gray-900 mb-2">
                {event.eventDate} at {event.time}
              </p>
              <p className="text-gray-900 mb-4 lg:mb-6">{event.eventLocation}</p>
              <p className="text-black">{event.eventDescription}</p>
              <Link href={`/registration/${event.id}`} className="inline-block mt-4 p-2 bg-blue-600 text-white font-bold py-2 px-4 rounded">
