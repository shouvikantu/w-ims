          ...
          {events.length > 0 ? (
            <ul className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              {events.map((event) => (
                <li
                  key={event.id}
                  className="bg-gray-900 bg-opacity-80 rounded-lg shadow-lg p-4 lg:p-6 "
                >
                  ...
                  <p className="text-gray-400 mb-4 lg:mb-6">
                    {event.eventLocation}
                  </p>
                  <p className="text-gray-300">{event.eventDescription}</p>
                  <Link
                    href={`/registration/${event.id}`}
                    className="inline-block mt-4 text-blue-500 hover:text-blue-300"
                  >
